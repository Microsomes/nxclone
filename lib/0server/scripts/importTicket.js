...
pool.getConnection(function (err, connection) {
    ...
    connection.query("TRUNCATE ticket_purchased");

    connection.query("CREATE TABLE IF NOT EXISTS ticket_status (id INTEGER PRIMARY KEY AUTO_INCREMENT, ticket_id INTEGER, status varchar(255), statusCreated TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",(err,result)=>{
        if (err) throw err;
        console.log("Table created: ticket_status");
    })
       
    connection.query("TRUNCATE ticket_status");
    console.log("----")
